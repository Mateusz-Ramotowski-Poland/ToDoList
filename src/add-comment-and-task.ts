...
export const taskArray1: TaskArray[] = [];
...
export function createNewTaskString(
  title: string | undefined,
  date: string,
  id: number
): string {
...
function addNewTask(event: Event): void {
  ...
  const newTask: string = createNewTaskString(
    textInputTask?.value,
...
function addNewComment(event: Event): void {
  ...
  const inputAuthor: HTMLInputElement | null =
    currentTask?.querySelector("[name='input-author']") || null;
  const inputComment: HTMLInputElement | null =
    currentTask?.querySelector("[name='input-comment']") || null;
  const comments: HTMLDivElement | null =
    currentTask?.querySelector(".comments") || null;
